...
test.describe('SSH Key Fingerprint', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(codeInput.byName(page, 'ssh-key-input').locator('.cm-editor')).toBeVisible({ timeout: 5000 })
  })
  ...
  test('paste valid ssh-rsa key → SHA256 and MD5 fingerprints displayed, key type and bit size shown', async ({
    ...
    await codeInput.fill(page, 'ssh-key-input', TEST_RSA_KEY)
  ...
  test('paste ssh-ed25519 key → fingerprints displayed, key type and bits shown', async ({ page }) => {
    await codeInput.fill(page, 'ssh-key-input', TEST_ED25519_KEY)
  ...
  test('paste invalid key → error toast shown', async ({ page }) => {
    await codeInput.fill(page, 'ssh-key-input', 'not a valid key')
  ...
  test('mobile viewport (375px) responsiveness', async ({ page }) => {
    ...
    await expect(codeInput.byName(page, 'ssh-key-input').locator('.cm-editor')).toBeVisible()
