...
describe('bcrypt Hash (cost 4)', () => {
  bench('WASM bcrypt hash', async () => {
    const { hashPassword } = await import('../src/wasm/bcrypt')
    await hashPassword('testpassword', 4)
  })
})

describe('bcrypt Verify (cost 4)', () => {
  bench('WASM bcrypt hash+verify', async () => {
    const { hashPassword, verifyPassword } = await import('../src/wasm/bcrypt')
...
describe('HMAC-SHA256', () => {
  ...
  bench('WASM hmac_sha256 (short)', async () => {
    const { generateHmac } = await import('../src/wasm/hmac')
